...
namespace lingvo.ner
{
    ...
    unsafe public sealed class NerScriber : IDisposable
    {
        ...
        private NerScriber( string modelFilename, string templateFilename )
		{
            ...
            {
                throw (new InvalidDataException( "Failed to open CRF-model." )); //throw (new InvalidDataException( "Не удалось открыть CRF-модель." ));
            }
        ...
        private void FreePinnedWordsBuffer()
        {
        ...
        private void AppendAttrValue( int wordIndex, CRFAttribute attr )
        {
        ...
        private void AppendAttrValue4ModelBuilder( int wordIndex, CRFAttribute attr )
        {
            switch ( attr.AttributeName )
            {
                case 'w':
